...
class lc_taxonomy extends WP_Widget {
	...
	function lc_taxonomy() {
		parent::WP_Widget( 'lc_taxonomy', $name = theme_name .__(' - List Custom Taxonomy','cmp') );
	}
	/**
	* This is the Widget
	**/
	function widget( $args, $instance ) {
		...
		if($dropdown){
			...
			$args = array(
					...
					'hierarchical'       => $hierarchical,
					'title_li'           => '',
					'show_option_none'   => __('No Categories','cmp'),
					'number'             => null,
					'echo'               => 1,
					'depth'              => 0,
					//'current_category'   => 0,
	...
	**/
	function form( $instance ) {
		...
		// The widget form ?>
			<p>
				<label for="<?php echo $this->get_field_id('title'); ?>"><?php echo __( 'Title:','cmp' ); ?></label>
				<input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" class="widefat" />
			</p>
			<p>
				<label for="<?php echo $this->get_field_id('taxonomy'); ?>"><?php echo __( 'Select Taxonomy:' ,'cmp'); ?></label>
			...
			<h4 class="lctw-expand-options"><a href="javascript:void(0)" onclick="lctwExpand('<?php echo $this->get_field_id('expandoptions'); ?>')" ><?php _e('More Options...','cmp') ?></a></h4>
			<div class="lctw-all-options">
				<h4 class="lctw-contract-options"><a href="javascript:void(0)" onclick="lctwContract('<?php echo $this->get_field_id('expandoptions'); ?>')" ><?php _e('Hide Extended Options','cmp') ?></a></h4>
				...
				<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Show post counts','cmp' ); ?></label><br />
				<input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id('hierarchical'); ?>" name="<?php echo $this->get_field_name('hierarchical'); ?>"<?php checked( $hierarchical ); ?> />
				<label for="<?php echo $this->get_field_id('hierarchical'); ?>"><?php _e( 'Show hierarchy','cmp' ); ?></label></p>
				<p>
					<label for="<?php echo $this->get_field_id('orderby'); ?>"><?php echo __( 'Order By:' ,'cmp'); ?></label>
				...
				<p>
					<label><input type="radio" name="<?php echo $this->get_field_name('ascdsc'); ?>" value="asc" <?php if( $ascdsc == 'asc' ) { echo 'checked'; } ?>/><?php _e('Ascending','cmp'); ?></label><br/>
					<label><input type="radio" name="<?php echo $this->get_field_name('ascdsc'); ?>" value="desc" <?php if( $ascdsc == 'desc' ) { echo 'checked'; } ?>/> <?php _e('Descending','cmp'); ?></label>
				</p>
				<p>
					<label for="<?php echo $this->get_field_id('exclude'); ?>"><?php _e('Exclude (comma-separated list of ids to exclude)','cmp'); ?></label><br/>
					<input type="text" class="widefat" name="<?php echo $this->get_field_name('exclude'); ?>" value="<?php echo $exclude; ?>" />
				</p>
				<p>
					<label for="<?php echo $this->get_field_id('exclude'); ?>"><?php _e('Only Show Children of (category id)','cmp'); ?></label><br/>
				...
				<label for="<?php echo $this->get_field_id('dropdown'); ?>"><?php _e( 'Display as Dropdown','cmp'); ?></label></p>
			</div>
<?php
	}
} // class lc_taxonomy
